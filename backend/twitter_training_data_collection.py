...
def did_inc(date):
    ...
    with conn.cursor() as cur:
        ...
        try:
            cur.execute(
                f"SELECT * FROM stocks where datetime < '{date}' ORDER BY datetime DESC LIMIT 1"
            )
        ...
        try:
            cur.execute(
                f"SELECT * FROM stocks where datetime > '{date}' ORDER BY datetime ASC LIMIT 1"
            )
...
with conn.cursor() as cur:
    ...
    for message in messages:
        ...
        cur.execute(
            f"INSERT INTO tweets (ticker, datetime, text, inc) VALUES('{ticker}', '{datetime}', '{text}', '{inc_val}')"
        )
