...
with conn.cursor() as cur:
    ...
    cur.execute(
        "CREATE TABLE IF NOT EXISTS TweetTable (id INT PRIMARY KEY, tweetTime TIMESTAMP , tweetText CHAR(280), company CHAR(4));"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS StockTable (id INT PRIMARY KEY, valueTime TIMESTAMP , value INT, company CHAR(4));"
    )
    conn.commit()
    conn.close()
