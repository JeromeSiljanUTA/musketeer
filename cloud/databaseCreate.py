...
with conn.cursor() as cur:
    ...
    cur.execute(
        "CREATE TABLE IF NOT EXISTS TweetTable (id INT PRIMARY KEY, tweetTime TIMESTAMP , tweetText CHAR(300), company CHAR(10));"
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS StockTable (id INT PRIMARY KEY, valueTime TIMESTAMP , value INT, company CHAR(10));"
    )
    conn.commit()
    conn.close()
